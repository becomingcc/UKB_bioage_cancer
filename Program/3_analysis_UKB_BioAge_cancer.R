...
plot_cox_tv_phenoage_anycancer <- ggplot(data.frame(
  ...
  #geom_segment(aes(x=startage, y=hr, xend=endage, yend=hr), size = .8, color="#0072B2") +
  ...
  #scale_fill_manual(name = "", values = c("95% CI Time-varying HR"=rgb(0, 0, 100, max = 100, alpha = 15))) + 
  theme_bw() +
...
#HR_BA_cancer <- read.delim("Output/Cox_BA_cancers.txt")
jpeg(file="Output/Cox_biomarkers_cancers.jpg", width = 18, height = 16, units = "cm", res = 1000, quality=100)
ggforestplot::forestplot(
  df = rbind(HR_biomarker_cancer %>% 
    ...
    mutate(name=c(rep(names(biomarkers),6),
                  rep(c("KDM residual","PhenoAge residual","HD"), 6)),
...
for (j in 1:length(BA_measures_2)) {
  ...
  ###### Melanoma ###### 
  coef <- summary(coxph(Surv((interview_date-birth_date)/365.25, (censor_date_mel-birth_date)/365.25, mel_diag) ~ . , 
                        ukb_bioage_cancer[(ukb_bioage_cancer$censor_date_any_cancer-ukb_bioage_cancer$interview_date)/365.25 >= 2 & ukb_biomarker_cancer$sun_protection!="missing"&ukb_biomarker_cancer$ethnicity_4group!="missing",c("interview_date","birth_date","censor_date_mel","mel_diag",BA_measures_2[j],cov_ukb_mel)]))
